...
file_path = "srtm_dem/{lat}{lon}.hgt"
...
class DEM:
    ...
    def __init__(self, lat, lon):
        ...
        with open(dem_file, 'r') as dem:
            elevations = np.fromfile(dem, dtype=np.dtype('>i2')).reshape(DEM._shape)
...
def main():
    ...
    dem = DEM(44.51, -80.3)
    ax.pcolormesh(dem.latlon[0], dem.latlon[1][::-1], dem.elevations[::-1])

    ax.set_xlim(dem.longitudes[0], dem.longitudes[-1])
    ax.set_ylim(dem.latitudes[-1], dem.latitudes[0])
    fig.savefig('{}.png'.format(dem.srtm_dem), dpi=600)
